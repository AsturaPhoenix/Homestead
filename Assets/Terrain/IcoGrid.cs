...
public class IcoGrid<T>
{
  ...
  private readonly int northTropic, southTropic;
  ...
  public IcoGrid(int subdivisions)
    ...
    data = new T[10 * subdivisions * subdivisions + 2];

    northTropic = PyramidSize(subdivisions);
    southTropic = northTropic + 5 * subdivisions * subdivisions;
  }
          ...
          for (int facePair = 0; facePair < 5; ++facePair) {
            ...
            Vector2 qa = icoNormals[facePair], qb = icoNormals[next], qc = icoNormals[facePair + 5], qd = icoNormals[next + 5];
            {
              ...
              for (int div = 0; div < len; ++div) {
                ...
                yield return new Vector3(v.x, y, v.y);
              }
            }
            {
